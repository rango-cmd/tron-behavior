...
def profile_single_wallet(file_path):
    ...
    address = os.path.basename(file_path).replace("_1hop.csv", "").strip()
    
    for col in ["From", "To", "Asset"]:
    ...
    # 1. lifespan_days
    lifespan_days = max((df["Time"].max() - df["Time"].min()).days, 1)
    ...
    if not df_usdt.empty:
        ...
        usdt_active_day_intensity = len(df_usdt) / usdt_active_dates if usdt_active_dates > 0 else 0.0
    else:
        df_usdt["date_str"] = ""
    
    # 3. dwell_time_avg_sec
    dwell_times, last_in_time = [], None
    ...
    for _, row in df_usdt.iterrows():
        if row["To"] == wallet_address:
            subseq_out = df_usdt[(df_usdt["Time"] > row["Time"]) & (df_usdt["Time"] <= row["Time"] + pd.Timedelta(hours=2)) & (df_usdt["From"] == wallet_address)]
            if len(subseq_out) >= 3 and row["Amount"] > subseq_out["Amount"].max():
                peeling_chain_count += 1
            if not subseq_out[np.abs(subseq_out["Amount"] - row["Amount"]) < 1e-2].empty:
                transit_pass_through_count += 1
                
        if row["From"] == wallet_address:
            prior_in = df_usdt[(df_usdt["Time"] >= row["Time"] - pd.Timedelta(hours=2)) & (df_usdt["Time"] < row["Time"]) & (df_usdt["To"] == wallet_address)]
            if len(prior_in) >= 3 and row["Amount"] > prior_in["Amount"].max():
                aggregation_count += 1

    # 7, 8, 9. balance metrics
    daily_net_retention_avg, balance_zero_days_ratio, balance_sawtooth_score = 0.0, 0.0, 0.0
    ...
    if not df_my_usdt.empty:
        ...
        daily_retentions, balance_history, current_balance, zero_balance_days = [], [], 0.0, 0
        ...
        for d_str in unique_dates:
            ...
            daily_retentions.append((day_in - day_out) / (day_in + day_out + 1e-6))
    ...
    if len(df_usdt) > 1:
        for i in range(len(df_usdt) - 1):
            tx1, tx2 = df_usdt.iloc[i], df_usdt.iloc[i+1]
            p1 = tx1["From"] if tx1["To"] == wallet_address else tx1["To"]
            p2 = tx2["From"] if tx2["To"] == wallet_address else tx2["To"]
            if p1 == p2 and ((tx2["Time"] - tx1["Time"]).total_seconds() / 60.0) <= 10.0:
    ...
    trx_distinct_src = trx_in["From"].nunique()
    trx_fuel_density = (trx_in["Amount"].sum() / trx_distinct_src) if trx_distinct_src > 0 else 0.0

    return {
        "address": address, "seed_label": seed_label, "lifespan_days": lifespan_days,
        "usdt_active_day_intensity": usdt_active_day_intensity, "dwell_time_avg_sec": dwell_time_avg_sec,
        "in_out_amount_ratio": in_out_amount_ratio, "peeling_chain_count": peeling_chain_count,
        "aggregation_count": aggregation_count, "transit_pass_through_count": transit_pass_through_count,
        "daily_net_retention_avg": daily_net_retention_avg, "balance_zero_days_ratio": balance_zero_days_ratio,
        "balance_sawtooth_score": balance_sawtooth_score, "otc_ping_pong_count": otc_ping_pong_count,
        "trx_in_max_amount": trx_in_max_amount, "trx_fuel_density": trx_fuel_density
    }
...
def main():
    ...
    if features_list:
        ...
        df_matrix[cols].to_csv(OUTPUT_PATH, index=False)
