...
# 💡 修正：輸出檔名明確指定為 gru_raw_model.pt
MODEL_SAVE_PATH = "data/gru_raw_model.pt"

SEQ_LEN = 50      
FEATURE_DIM = 5    

def build_raw_gru_dataset():
    ...
    for file_path in tqdm(csv_files, desc="Parsing raw transaction streams for GRU", unit="file"):
...
class TronRawGRUClassifier(nn.Module):
...
def main():
    print("🎬 啟動純 Raw Data 驅動型 GRU 時序運算引擎...")
    X, y = build_raw_gru_dataset()
    if X is None: return
    ...
    print(f"Loading GRU Model onto -> {device}")

    model = TronRawGRUClassifier(FEATURE_DIM, hidden_dim).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.BCEWithLogitsLoss()

    print("Starting GRU Sequential Training Loop...")
    ...
    print(f"Success: GRU model saved to -> {MODEL_SAVE_PATH}")
